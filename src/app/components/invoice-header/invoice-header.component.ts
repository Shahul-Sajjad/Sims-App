...
export class InvoiceHeaderComponent implements OnInit {
  ...
  getDistinctBillToGstNo() {
    // tslint:disable-next-line: max-line-length
    const dataRequest = `<queryString>select distinct * from (select gstNo +' ('+ state +')-'+ business_Place as billToGstNo,gstNo from dbo.mtr_gst_number where plant_Code!= '') as T</queryString>`;
    const soapRequest = this.commonService.getSoapBody('RunSqlQuery', 'http://schemas.cordys.com/WINDatabaseMetadata', dataRequest);
    this.simsHttpCoreServices.httpPost(soapRequest).subscribe(response => {
      let distinctBillToGstNO = this.responseDataManipulation(response, 'RunSqlQueryResponse');
      distinctBillToGstNO.forEach(element => {
        const gstNoList = {
          name: element.old[0].TABLE[0].billToGstNo[0],
          value: element.old[0].TABLE[0].gstNo[0]
        };
  ...
  getInvoiceHeaderDetails() {
    ...
    this.simsHttpCoreServices.httpPost(soapRequest).subscribe(response => {
      ...
      headerFormItems.forEach(element => {
        this.setValueLocalStorage(element);
        this.setInvoiceHeaderFormData(element);
        this.headerFormValidation();
        this.setValuesforPostToERPopup(element)
      });
    });
  }
  ...
  onBillGSTNoChange() {
    const billGSTNo = this.invoiceHeaderForm.value.billGST;
    const businessPlace = this.gstNoLists.find((billGSTInfo: any) => billGSTInfo.value === billGSTNo).name.split('-')[1];
    this.invoiceHeaderForm.patchValue({
      businessPlace
    });
  }
