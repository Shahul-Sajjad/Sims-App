...
(function(window, $, undefined) {
    ...
    $.cordys.addOrganizationContextToURL = function(url) {
        ...
        if ($.cordys.webDirectoryPath !== null) {
            ...
            var baseURLPrefix = 'http://111.93.191.82:96/home/system/';
    ...
    $.cordys.ajax = function(options) {
        ...
        opts.error = function(jqXHR, textStatus, errorThrown) {
            ...
            if (messCode.search(/Cordys.*(AccessDenied|Artifact_Unbound)/) >= 0 || jqXHR.statusText === "Forbidden") {
                ...
                if (!jqXHR.error().responseText && !jqXHR.error().responseXML) return;
                var showError = true;
                var errorMessage = $(jqXHR.error().responseXML).find("faultstring,error elem").text() || jqXHR.responseText;
...
(function(window, $, undefined) {
    var userDetails, preloginInfo = null,
        topLevelWindow = $.cordys.getTopLevelWindow(window),
    ...
    function showLoginPage() {
        ...
        if (authenticatorType === AUTH_TYPE.SAML2) {
            authID = decodeURIComponent($.cordys.getURLParameter(window.location, "authID")), currentOrganizationDN = getCurrentOrganizationDN();
...
(function(window, $, undefined) {
    ...
    function renewSAMLAssertions(loginResponse) {
        var userName = $(loginResponse).find('AssertionArtifact, saml\\:NameIdentifier').text(),
            ...
            preExpirationRefreshPeriodMs = 300000,
            minimalPeriodMs = 10000,
            expirationPeriodMs;
